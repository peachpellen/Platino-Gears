...
//Rare exceptions to indicate a bug in the _gears code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_gears = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_gears]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.gears.module"
